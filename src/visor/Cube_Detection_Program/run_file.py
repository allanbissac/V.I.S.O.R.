...
def main():
    ...
    # Input/Output
    source = 1  # Path to input video file or webcam index (0 for default camera)
    ...
    # Only keep these classes (by NAME, not by numeric id)
    TARGET_CLASS_NAMES = {"small cube", "medium cube", "large cube"}
        ...
        try:
            ...
            for detection in detections:
                try:
                    ...
                    # Only process cube classes
                    class_name_norm = _norm_class_name(class_name)
                    if class_name_norm not in TARGET_CLASS_NAMES:
                        continue

                    # For cubes, use the center point depth (stable on compact objects)
            ...
            for box_3d in boxes_3d:
                try:
                    # Determine color based on cube size
                    class_name = _norm_class_name(box_3d['class_name'])
                    if class_name == "small cube":
                        ...
                        color = (255, 255, 255)  # White (shouldn't happen due to filtering)
